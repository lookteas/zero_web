...
test('me page exposes trial-ready feature entries with clear status labels', () => {
  ...
  assert.equal(source.includes('SecurityPanel'), true)
  assert.equal(source.includes('<SecurityPanel passwordError={Boolean(query.passwordError)} />'), true)
  assert.equal(source.includes('已开放'), true)
  assert.equal(source.includes('即将上线'), true)
  assert.equal(source.includes('待开放'), false)

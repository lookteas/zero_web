...
test('free mode recent practices open one review panel instead of expanding the list', () => {
  assert.equal(freeModeWorkbench.includes('selectedPracticeId'), true)
  assert.equal(freeModeWorkbench.includes('function PracticeReviewPanel('), true)
  assert.equal(freeModeWorkbench.includes('练习回看'), true)
  ...
  assert.equal(freeModeWorkbench.includes('收起回看'), true)
  assert.equal(freeModeWorkbench.includes('practice.practiceId === selectedPracticeId ? null : practice.practiceId'), true)
  assert.equal(freeModeWorkbench.includes('updateFreemodePracticeAction'), true)
  assert.equal(freeModeWorkbench.includes('line-clamp-3'), true)
  assert.equal(freeModeWorkbench.includes('<AwarenessDetailReader summary={practice.awarenessSummary || "这条意识点暂无摘要。"} details={practice.awarenessDetails || ""} />'), false)

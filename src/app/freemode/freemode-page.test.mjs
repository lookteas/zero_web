...
test('free mode workbench uses a compact selection board instead of a mobile list', () => {
  ...
  assert.equal(freeModeWorkbench.includes('md:min-h-[148px]'), true)
  assert.equal(freeModeWorkbench.includes('getChapterIllustration'), true)
  assert.equal(freeModeWorkbench.includes('/freemode-chapters/chapter-'), true)
  assert.equal(freeModeWorkbench.includes('章节图'), true)
